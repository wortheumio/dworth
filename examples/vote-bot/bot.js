...
// setup the dworth client, you can use other nodes, for example gtg's public node at https://api.wortheum.news:8090
const client = new dworth.Client('https://api.wortheum.news')

// deserialize the posting key (in wif format, same format as you find on the wortheum.news interface)
const key = dworth.PrivateKey.from(POSTING_KEY)
...
const stream = client.blockchain.getOperationsStream({mode: dworth.BlockchainMode.Latest})

console.log(`Following ${ FOLLOW_USER } with ${ VOTE_WEIGHT / 100 }% vote weight`)

// the stream will emit one data event for every operatio that happens on the wortheum blockchain
    ...
    if (operation.op[0] == 'vote') {
        ...
        if (vote.voter === FOLLOW_USER) {
            ...
            client.broadcast.vote(vote, key).then(() => {
                console.log(`Voted for https://wortheum.news/@${ vote.author }/${ vote.permlink }`)
            }).catch((error) => {
