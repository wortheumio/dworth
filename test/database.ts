...
describe('database api', function() {
    ...
    it('getDynamicGlobalProperties', async function() {
        ...
        assert.deepEqual(Object.keys(result), [
            ...
            'total_vesting_fund_worth', 'total_vesting_shares', 'total_reward_fund_worth',
            'total_reward_shares2', 'pending_rewarded_vesting_shares', 'pending_rewarded_vesting_worth',
    ...
    it('getConfig', async function() {
        ...
        assert.equal(result['WORTH_CHAIN_ID'], client.options.chainId)
        serverConfig = result
        // also test some assumptions made throughout the code
        const conf = await liveClient.database.getConfig()
        assert.equal(conf['WORTH_CREATE_ACCOUNT_WITH_WORTH_MODIFIER'], 30)
        assert.equal(conf['WORTH_CREATE_ACCOUNT_DELEGATION_RATIO'], 5)
        assert.equal(conf['WORTH_100_PERCENT'], 10000)
        assert.equal(conf['WORTH_1_PERCENT'], 100)
    })
    ...
    it('getBlock', async function() {
        ...
        assert.equal('0000000000000000000000000000000000000000', result.previous)
        assert.equal(serverConfig['WORTH_INIT_PUBLIC_KEY_STR'], result.signing_key)
    })
    ...
    it('getDiscussions', async function() {
        const r1 = await liveClient.database.getDiscussions('comments', {
            ...
            start_permlink: 're-pal-re-almost-digital-dworth-a-strongly-typed-worth-client-library-20170702t131034262z',
            tag: 'almost-digital',
            limit: 1,
        })
    ...
    it('getChainProperties', async function() {
        ...
        assert.equal(Asset.from(props.account_creation_fee).symbol, 'WORTH')
    })

    it('getCurrentMedianHistoryPrice', async function() {
        const price = await liveClient.database.getCurrentMedianHistoryPrice()
        assert.equal(Asset.from(price.base).symbol, 'WBD')
        assert.equal(price.quote.symbol, 'WORTH')
    })
    ...
    it('getVestingDelegations', async function() {
        this.slow(5 * 1000)
        const [delegation] = await liveClient.database.getVestingDelegations('worth', '', 1)
        assert.equal(delegation.delegator, 'worth')
