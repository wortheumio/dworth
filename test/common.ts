...
export const TEST_NODE = process.env['TEST_NODE'] || 'https://api.wortheum.news'
...
export async function createAccount(): Promise<{username: string, password: string}> {
    ...
    const username = `dworth-${ randomString(9) }`
    const response = await fetch('https://testnet.wortheum.news/create', {
        method: 'POST',
