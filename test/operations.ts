...
describe('operations', function() {
    ...
    it('should transfer worth', async function() {
        ...
        assert.equal(Asset.from(acc2af.balance).subtract(acc2bf.balance).toString(), '0.042 WORTH')
    })
    ...
    it('should create account and post with options', async function() {
        ...
        await client.broadcast.commentWithOptions({
            ...
            percent_worth_dollars: 0,
            max_accepted_payout: Asset.from(10, 'WBD'),
            extensions: [
                [0, {beneficiaries: [
        ...
        // not sure why but on the testnet the recovery account is always 'wortheum'
        ...
        assert.equal(post.max_accepted_payout, '10.000 WBD')
        assert.equal(post.percent_worth_dollars, 0)
    ...
    it('should create account and calculate fees', async function() {
        ...
        await client.broadcast.createAccount({
            ...
            fee: '2.000 WORTH', delegation: Asset.from(1000, 'VESTS')
        }, acc1Key)
